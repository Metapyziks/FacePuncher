        ...
        public Level Generate(int seed)
        {
            ...
            for (int i = 0; i < 4; ++i) {
                for (int j = 0; j < 4; ++j) {
                    ...
                    room.AddWalls(new Rectangle(0, 0, room.Width, room.Height));
                    room.AddFloors(new Rectangle(1, 1, room.Width - 2, room.Height - 2));

                    if (i > 0) room.AddFloors(new Rectangle(0, 3, 1, 2));
                    if (j > 0) room.AddFloors(new Rectangle(3, 0, 2, 1));
                    if (i < 3) room.AddFloors(new Rectangle(7, 3, 1, 2));
                    if (j < 3) room.AddFloors(new Rectangle(3, 7, 2, 1));
